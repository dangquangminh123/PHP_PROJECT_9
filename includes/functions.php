...
//get_post này mặc định sẽ trả về 1 mảng luôn là 1 object
...
// get_posts này sẽ trả về 1 mảng nhưng có thể là object cũng có thể 1 mảng theo database (kiểu assoc)
function get_posts(array $args = [],string $type = 'object')
{
    ...
    if(!empty($args))
    {
        ...
        {
            $v = (string)$v;
            $condition_ar[] = "$k = '$v'";//type = 'class'; status = 'publish'
        }
        if ($condition_ar > 0) {
            $condition = "WHERE " . implode(" AND ", $condition_ar);
        }
